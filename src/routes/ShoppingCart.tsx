...
export default function ShoppingCart() {
    ...
    var { cartItems, totalPrice, getItemQuantity, addItem, reduceItem, deleteItem } = useShoppingCart()
    ...
    return (
        <div className="flex flex-col grow items-center">
            <div className={`flex flex-col relative grow px-10 py-20 lg:w-2/3 ${cartItems.length == 0 ? "" : "space-y-8"}`}>
                <p className="text-3xl">Cart</p>
                {
                    (cartItems.length == 0)
                        ...
                        : cartItems.map((e) => {
                            var itemInCardQuantity = getItemQuantity(e.product)
                            return (
                                <div className="flex shadow-md border-t-2">
                                    <img src={e.product.photoUrl} alt="" className="w-24 object-cover" />
                                    <div className="flex flex-col p-5 space-y-3 grow">
                                        <div className="flex flex-col space-y-1">
                                            <div className="flex">
                                                {
                                                    (e.product.isSpecial)
                                                        ? <p className="px-5 py-1 bg-red-100 text-red-800  rounded-md ">Special</p>
                                                        : <></>
                                                }
                                            </div>
                                            <p>
                                        ...
                                        <div className="flex items-center w-full">
                                            <div className="flex items-center space-x-3 grow">
                                                <SoilButton outlined onClick={() => reduceItem(e.product)}>
                                                    -
                                                </SoilButton>
                                            ...
                                            </div>
                                            {
                                                (itemInCardQuantity > 0)
                                                    ? <div className="">
                                                        <Link to={""} onClick={() => deleteItem(e.product)} className="text-red-600">
                                                            <img src={Trash} alt="" />
                                                        </Link>
                                                    </div>
                                                    : <></>

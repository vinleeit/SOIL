...
export default function SoilLogo({
  ...
  const sizeValue =
    size == "large" ? "36px" : size == "medium" ? "32px" : "24px";
  return (
    <Link to={"/"} onClick={onClick}>
      <h2
        className={`${size == "large" ? "text-2xl" : size == "medium" ? "text-xl" : "text-lg"} font-bold flex ${vertical ? "flex-col" : "space-x-1.5"} items-center justify-center`}
      >
        <svg
          width={sizeValue}
          height={sizeValue}
